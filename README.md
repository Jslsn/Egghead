...
Definitely a bit overkill for what it is and not worthy of the arc name for now, but will update it to make things cleaner. 
...
- Fill in the following variables and secrets with their respective values:
    - Variables:
        - AWS_REGION (Region you want things deploying in)
        - BUCKET_NAME (How you want the bucket to be named (It will be suffixed with a datetime))
        - DOMAIN_NAME (The domain you own in route53)
    - Secrets:
        - ACM_ARN (ARN for the certificate you have)
        - AWS_ROLE (ARN for the role github is to assume in aws)
        - AWS_S3_BUCKET (Name of the bucket you want to store your tfstate)
- Push to main
